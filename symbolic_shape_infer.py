...
class SymbolicShapeInference:
    ...
    def _preprocess(self, in_mp):
        ...
        #print('self.known_vi_ :'); print(self.known_vi_);   exit(0) 
    ...
    def _infer_Expand(self, node):
        ...
        if expand_to_shape is not None:
            ...
            #print('222')
            print('shape :', shape)
            print('expand_to_shape :', expand_to_shape);    exit(0)
    ...
    def _infer_Slice(self, node):
        ...
        def handle_negative_index(index, bound):
            ...
            try:
                if not less_equal(0, index):
                    print('index :', index);    print('bound :', bound);    exit(0)
        ...
        if starts is None or ends is None:
            ...
            #print('\nends :', ends);  #exit(0)
            #print('starts :', starts);  #exit(0)
            #print('axes :', axes);  #exit(0)
            #print('steps :', steps);  #exit(0)
    ...
    def _infer_impl(self, start_sympy_data=None):
        ...
        #print('type(self.out_mp_.graph.input) :', type(self.out_mp_.graph.input)); # exit(0)
        #print('len(self.out_mp_.graph.input) :', len(self.out_mp_.graph.input));  exit(0)
        for idx, i in enumerate(self.out_mp_.graph.input):
            #print('idx : {} / {}'.format(idx, len(self.out_mp_.graph.input)))
        ...
        for node in sorted_nodes:
            ...
            for i_o in range(len(node.output)):
                vi = self.known_vi_[node.output[i_o]]
                #print('node.output[i_o] :', node.output[i_o]);  print('vi :', vi);
                out_type = vi.type
    ...
    @staticmethod
    def infer_shapes(in_mp, precision, int_max=2**31 - 1, auto_merge=False, guess_output_rank=False, verbose=0):
        ...
        #kount = 0;
        while symbolic_shape_inference.run_:
            #print('kount :', kount);    kount += 1;
            all_shapes_inferred = symbolic_shape_inference._infer_impl()
        symbolic_shape_inference._update_output_from_vi()
        if not all_shapes_inferred:
            raise Exception("Incomplete symbolic shape inference")
        if 'float16' == precision.lower():
            print('in_mp :\n{}'.format(in_mp.graph.value_info)); 
            print('out_mp :\n{}'.format(symbolic_shape_inference.out_mp_.graph.value_info));
        else:
            print('in_mp :\n{}'.format(onnx.helper.printable_graph(in_mp.graph))); 
            print('out_mp :\n{}'.format(onnx.helper.printable_graph(symbolic_shape_inference.out_mp_.graph)));
        return symbolic_shape_inference.out_mp_


def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument("--precision", required=True, type = str, help="Things like float32, float16 ...")
...
if __name__ == "__main__":
    ...
    if args.output and out_mp:
        ...
        print('The shape-infered onxx is saved at {}'.format(args.output))
        logger.info("Done!")
